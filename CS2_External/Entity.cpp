...
bool CEntity::UpdatePawn(const DWORD64& PlayerPawnAddress)
{
	...
	if (!this->Pawn.GetHealth())
		return false;
	if (!this->Pawn.GetHasHeavyArmour())
		return false;
	if (!this->Pawn.GetHelmet())
		return false;
	if (!this->Pawn.IsWalkingGV())
		return false;
...
//broken dont use
bool PlayerPawn::GetHasHeavyArmour() {
	return ProcessMgr.ReadMemory(Address + Offset::Pawn.m_bHasHeavyArmor, this->HasHeavy);
}
bool PlayerPawn::GetHelmet() {
...
//broken dont use
bool PlayerPawn::IsWalkingGV() {
	return ProcessMgr.ReadMemory(Address + Offset::Pawn.m_bIsWalking, this->IsWalking);
}
